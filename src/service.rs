...
pub struct DiskService {
    drives: HashMap<u32, DiskType>,
    next_id: u32,
}
impl DiskService {
    pub fn new() -> Self { Self { drives: HashMap::new(), next_id: 1 } }
    pub fn init(&mut self) {
        for bus in 0..2 {
            for device in 0..2 {
                unsafe {
                    if let Some(info) = ide_identify(bus, device) {
                        self.drives.insert(self.next_id, DiskType::ATA(AtaDrive::from_identify(info, bus, device)));
                        self.next_id += 1;
                    }
                }
            }
    ...
    pub fn get(&self, id: u32) -> Option<&DiskType> {
        match self.drives.get(&id) {
            None => None,
            Some(dt) => Some(&dt)
        }
    }
    pub fn iter(&self) -> hashbrown::hash_map::Iter<u32, DiskType> {
        self.drives.iter()
    }
}

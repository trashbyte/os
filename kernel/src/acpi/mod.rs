...
use acpi_crate::{AcpiHandler, PhysicalMapping, parse_rsdp};
...
pub fn init() {
    ...
    if let InterruptModel::Apic(a) = apic_slot {
        *(crate::apic::LOCAL_APIC.lock()) = APIC::new(a.local_apic_address);
    }
    else {
    ...
    //aml_context.parse_table(unsafe { alloc::slice::from_raw_parts((dsdt.address as u64 + PHYS_MEM_OFFSET) as *const u8, dsdt.length as usize) }).unwrap();
}
